...
if __name__ == "__main__":
    ...
    prob = label @ Train_X / N_train + 1e-100
    prob = np.log(prob)
    ...
    for i in range(10):
        predict[i] = np.log(freq[i]) + np.multiply(prob[i],Test_X==1).sum(axis=1) 

    for i in range(N_test):
        predict_y[i] = np.where(predict[:,i] == np.max(predict[:,i]))[0][0]

    for i in range(10):
        train_predict[i] = np.log(freq[i]) + np.multiply(prob[i],Train_X==1).sum(axis=1) 

    for i in range(N_train):
        train_predict_y[i] = np.where(train_predict[:,i] == np.max(train_predict[:,i]))[0][0]
        
    print("Complete!")

    accuracy = np.mean(predict_y == test_y)
    print("Test Error Rate: "+str(1-accuracy))
    accuracy = np.mean(train_predict_y == train_y)
    print("Train Error Rate: "+str(1-accuracy))
